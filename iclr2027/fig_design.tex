...
\begin{axis}[
  ...
  legend style={at={(0.97,0.04)}, anchor=south east, draw=none, fill=none,
  ...
  \addplot [formalcol, mark=*, mark size=1.3pt] coordinates {(0,0.393) (1,0.405) (2,0.500) (3,0.486) (4,0.516)};
  \addplot [natcol, mark=square*, mark size=1.3pt] coordinates {(0,0.393) (1,0.403) (2,0.490) (3,0.532)};
  \legend{Formal, English}
\end{axis}
\end{tikzpicture}
\caption{Accuracy on ProofWriter at inference depth 3. The grey band gives the
range over the three seeds of the unmodified control.}
\label{fig:overview-b}
\end{subfigure}
\caption{The data added to the instruction mixture and the accuracy it buys.
Replacing 1\% of the mixture already lets the model write derivations that a
checker accepts, and 5\% gives the full gain on a held-out deduction
benchmark.}
\label{fig:overview}
\end{figure}
