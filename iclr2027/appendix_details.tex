...
is held fixed across conditions. Seeds 3407, 3408 and 3409 give the three
replicates on Qwen2.5-7B, and only the final checkpoint is kept. Before
...
default prompts of the evaluation harness. The three seeds of the Qwen2.5-7B
control span 0.3 points on GSM8K, 0.1 on MMLU, 0.8 on ARC-Challenge, 0.6 on
LogiQA, 0.0 on HellaSwag, 0.4 on PIQA and 1.0 on WinoGrande.
...
between ``true'' and ``false'' on Qwen2.5-7B, seed 3407 with seed 3408 in
parentheses. The pooled column mixes the two polarities and the next two
columns rank within a polarity. \longdoc{} is a length-matched control that
...
and 24.2 percent for the English mixture, with the second seed within half a
...
items falls from 0.53 to 0.27 once the supporting facts are gone, and the
control falls from 0.35 to 0.20. The English mixture retains more, falling
from 0.67 to 0.46, so about two thirds of its advantage on this cell
...
percent English derivations both answer correctly, all at seed 3407. The
...
training with sixteen samples per prompt and one greedy decode.
...
$78.5\pm14.9$ for \nat{}), and a replication at a second seed. Concatenating
...
same instruction tuning to every condition. That route also produces the
gain on ProofWriter, 12.6 points for the English rendering at inference
depth 3 with two instruction-tuning seeds, and it also leaves the general
benchmarks flat apart from a loss of two to three points on LogiQA that
replicates across seeds. Midtraining is the more expensive route and buys
no more, since the instruction-tuning route reaches the same accuracy,
generalises further on long chains, carries no cost on LogiQA, and adds
nothing when the two are applied in sequence. One ability survives
midtraining that instruction tuning without derivations removes completely,
since the midtrained model writes checkable proofs up to chain length 25
when prompted in the format of its training documents, and writes none
after ordinary instruction tuning.
