...
\subsection{A negated ProofWriter item}
\label{sec:pw-example}

In a typical item of the kind discussed in Section~\ref{sec:prior} the theory
contains "If someone is young then they visit the dog. Red people are young.
If the rabbit visits the dog then the dog is red." together with facts from
which the rabbit is red, and the claim is "The dog is not red". The baseline
...
end. Both models write out the full state after every swap. The baseline
...
and answer orange. On a second item the baseline reaches the correct final
...
for the number of loops. The baseline reads the exponent as the loop count and
...
The item below shows the failure of the Qwen2.5-7B baseline on web of lies
