...
Every model trains for one epoch on $10^5$ examples, except in the size
ablation of Table~\ref{tab:size}, full-parameter under
...
training starts, the mixture is tokenized and the examples with a target that
...
derivations with their conclusions removed. No run aborted. Qwen2.5-32B
...
as a chain-of-thought few-shot task with an answer extractor that reads a plain
...
documents and scores the fraction of items for which the checker accepts
the derivation and the answer is correct.
...
deduction (three, five and seven objects) and formal fallacies. GPQA-Diamond
contains questions that require computing a quantity through several
dependent steps and questions that test recall of a concept, and only the
first kind involves a procedure of the kind the derivations teach. We
separate the two with a rule that reads only the question text, so that
neither the gold answers nor the outputs of any model enter the split, and
anyone can recompute the subset from the public dataset. An item is
quantitative if its question, matched without regard to case, contains a
number followed by a unit,
\begin{alltt}\footnotesize
\textbackslash{}b\textbackslash{}d+(\textbackslash{}.\textbackslash{}d+)?\textbackslash{}s*(nm|mm|cm|km|kg|mol|ml|eV|keV|MeV|GeV|K|Hz|MHz|GHz
    |deg|\textdegree|%|pc|AU|M_|R_)
\end{alltt}
or contains a digit and matches
\begin{alltt}\footnotesize
\textbackslash{}b(what is the (value|ratio|factor|energy|wavelength|number|probability
    |concentration|pH|mass|radius|velocity|temperature)|calculate|compute
    |how many|by what factor|what fraction)\textbackslash{}b
\end{alltt}
where the line breaks are for display only. The rule assigns 73 of the 198
items to the quantitative subset and 125 to the remainder. The sign test
compares, for each item, the pooled runs trained with derivations against
the pooled baseline runs.
...
baseline span 0.3 points on GSM8K, 0.1 on MMLU, 0.8 on ARC-Challenge, 0.6 on
...
Every mixture in the main text contains a single rendering of the derivations.
Table~\ref{tab:mixed} reports mixtures that contain both renderings of the same
...
depth 5 and 5.3 points on FOLIO over the baseline, which is at the level of the
...
With the notation instruction, the model writes the notation that the
prompt asks for. The conditioned models write a derivation the checker accepts under either instruction, at 96.5 to
...
two polarities and the next two columns rank within a polarity. \longdoc{} is a length-matched baseline that
...
identically. The baseline does so on 37.6 percent of decidable items and the
length-matched baseline on 37.6 percent, against 28.5 percent for the formal
...
baseline falls from 35.0 to 20.0 percent. The English mixture retains more, falling
...
gives a smaller gain that is more firmly tied to the premises, and the
...
Each example below is an item that the Qwen2.5-7B baseline answers
...
the rabbit. The baseline answers ``True'' and both trained models answer
...
is false. The baseline answers ``Uncertain'' and both trained models answer
...
conductors.'' and the claim ``Beethoven is not a conductor''. The baseline
...
\textbf{Baseline.} (1) Jim lies. (2) Since Jim lies and Yoland says Jim lies,
...
\paragraph{Tracking shuffled objects.} Beyond the item in
Section~\ref{sec:procedure}, on an item with 5 holders and 5 swaps the
baseline reaches the correct final state and then selects the wrong option
letter, which the trained models do not.
...
baseline writes that the field ``is the same as if the charge were
...
and answer $q/(4\pi\epsilon_0 L^2)$. 
Section~\ref{sec:procedure} gives a second item from this benchmark.
...
own problems, a derivation the checker accepts contains the correct answer in
...
An experiment on the generator problems alone shows that the ordering of the two
...
depth. It survives a baseline that equates the number of supervised target
tokens by subsampling the longer condition, a baseline that removes a schema
...
same instruction tuning to every condition. The conditions are the baseline,
a length-matched baseline that replaces the same tokens by ordinary long
...
with the gold label false. It also leaves the general benchmarks flat apart
from a loss of 2 to 3 points on LogiQA that the second run reproduces.
Midtraining is the more expensive route and gives no further gain, since the
instruction-tuning route reaches the same accuracy, generalises further on
long chains, does not lower LogiQA accuracy, and adds nothing when the two are
...
in the format of their training documents, and the baseline base writes
