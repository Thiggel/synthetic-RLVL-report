...
at all. We train the plain form at shares of 2, 5 and 10 percent and the
conditioned form at 2 and 5 percent, on Dolci and on the same base model as
...
less and are not ordered by their share. At the matched share of 5 percent the
conditioned mixture is 2.4 points above the plain one at inference depth 3 and
1.5 points below it on FOLIO.
...
100 percent in both renderings and at both shares, and they write no
derivation at all when neither instruction is present. The plain model at 2
percent writes an accepted derivation when it is left to choose, and it also
follows the formal instruction in form, with a formal block in 100 percent of
its answers at chain length 25, but the checker accepts 1.5 percent of those
derivations. Training on both renderings without a marker therefore leaves a
model that can imitate the notation it is asked for without being able to
derive in it, which the one added sentence repairs.
