\begin{table}[H]
\caption{Intermediate values in web of lies. The model writes 1 numbered line per person, so the truth value it carries can be read at each position and compared with the value the chain implies. The last two columns give the final answer accuracy when every intermediate value is correct and when at least one is not.}
\label{tab:steps}
\begin{center}
\small
